// index.js (Backend Final & Lengkap Sebenarnya)
...
app.delete('/api/attachments/:id', async (req, res) => {
    try {
        await pool.query('DELETE FROM attachments WHERE id_attachment = $1', [req.params.id]);
        res.status(200).json({ message: 'Attachment berhasil dihapus' });
    } catch (err) { res.status(500).json({ error: err.message }); }
});

// =================================================================
// API ENDPOINT: DASHBOARD & READ-ONLY
// =================================================================
...
app.put('/api/po/:id', upload.array('attachments', 10), async (req, res) => {
    try {
        ...
        const oldData = await pool.query('SELECT * FROM purchase_order WHERE no_po = $1', [id]);

        const query = 'UPDATE purchase_order SET perihal_project = $1, nominal = $2, status_po = $3 WHERE no_po = $4';
        const values = [ perihal_project, nominal, status_po, id ];
        await pool.query(query, values);
...
app.put('/api/memo/:id', upload.array('attachments', 10), async (req, res) => {
    try {
        ...
        const query = 'UPDATE memo_procurement SET perihal = $1 WHERE no_memo = $2';
        const values = [ perihal, id ];
        await pool.query(query, values);
...
// =================================================================
// API ENDPOINTS: INVOICE (Read-only)
// =================================================================
